...
class LOTClassTrainer(object):
    ...
    def __init__(self, args):
        ...
        self.pretrained_lm = 'bert-base-chinese'
    ...
    def filter_keywords(self, category_vocab_size=100):
        ...
        sorted_dicts = {}
        #comment: sort the category_words_freq by how often the word appear
        ...
        repeat_words = []
        #comment: figure out which words appear in more than one category
        for word_id in all_words:
            if len(all_words[word_id]) > 1:
                repeat_words.append(word_id)
        self.category_vocab = {}
        #comment: store sorted_dicts as self.category_vocab and in numpy format and remove the frequency 
        ...
        for i, word_list in self.category_vocab.items():
            ...
            for j, word_id in enumerate(word_list):
                ...
                #     delete_idx.append(j)
                # if word == '[UNK]' or word == 'silver' or word == 'wto':
                #   delete_idx.append(j)

                if word in self.label_name_dict[i]:
                    continue
                if word_id in repeat_words or word == '、' or re.search("^[a-zA-Z0-9?><：:;,.、{}[\]\-_+=!@#$%\^&*|']*$", word):
                    delete_idx.append(j)
            self.category_vocab[i] = np.delete(self.category_vocab[i], delete_idx)
        

    # construct category vocabulary (distributed function)
    def category_vocabulary_dist(self, rank, top_pred_num=45, loader_name="category_vocab.pt"):
    ...
    def category_vocabulary(self, top_pred_num=45, category_vocab_size=100, loader_name="category_vocab.pt"):
    ...
    def prepare_mcp_dist(self, rank, top_pred_num=50, match_threshold=15, loader_name="mcp_train.pt"):
        model = self.set_up_dist(rank)
        model.eval()
    ...
    def prepare_mcp(self, top_pred_num=50, match_threshold=15, loader_name="mcp_train.pt"):
        loader_file = os.path.join(self.dataset_dir, loader_name)
        if os.path.exists(loader_file):
